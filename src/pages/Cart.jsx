...
const Cart = () => {
    ...
    const { products,axios,user, currency, cartItems,setCartItems, removeFromCart, getCartCount, updateCartItem, navigate, getCartAmount } = useAppContext()
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {
                                navigate(`/products/${product.cartegory.toLowerCase()}/${product._id}`); scrollTo(0, 0)
                        ...
                        <button onClick={() => removeFromCart()} className="cursor-pointer mx-auto">
                            <img src={assets.remove_icon} className="inline-block h-6 w-6" alt="remove" />
                        </button>
                    </div>)
                )}
